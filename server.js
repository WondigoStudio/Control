...
// Лёгкий эндпоинт специально для регистрации активности "просмотра" —
// открытие/закрытие карточки монитора не мутирует никаких данных, но с
// точки зрения человека "я открыл дашборд и что-то смотрю" — это тоже
// активность, и это осознанный клик (в отличие от фонового опроса).
// Сама запись активности уже происходит в общем middleware выше (он ловит
// любой POST /api/*) — этот роут просто даёт фронтенду легальный повод
// сделать такой POST для чисто "просмотровых" действий.
